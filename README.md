...
#### 8. 树洞养料站 (Tree Hole Nutrition Station) - *每日签到系统*
一个充满仪式感的每日签到与奖励系统，鼓励用户持续记录生活。

*   **📅 视觉化日历交互**：
    *   **种子情报**：日历不再是冰冷的数字，每日展示**随机种子数量情报**（如 `🌱 +12`），基于日期哈希的确定性算法，既保证了随机的趣味性，又确保了同一天数据的稳定性。
    *   **状态追踪**：直观展示已签到、漏签、今日待签等多种状态，漏签日期会自动标记提示补签。
    *   **沉浸式 UI**：采用半透明毛玻璃风格设计，配合动态种子图标与金色收获色调，营造温暖的视觉体验。

*   **🎁 激励机制**：
    *   **连续签到奖励**：可视化的进度条展示 7天、14天、30天 的连续签到里程碑，达到目标即可解锁额外种子奖励。
    *   **种子货币体系**：签到获得的“种子”将作为社区内的流通货币，未来可用于兑换装扮或解锁特殊功能。

*   **⚡ 极致性能**：
    *   **零依赖架构**：彻底移除了不稳定的外部 CDN 依赖（如农历 API），所有核心逻辑（包括日历生成、种子算法）均由原生 JS 实现，确保了极高的加载速度与稳定性。

#### 9. 漂流树洞 (Drifting Bottle) - *缘分的随机碰撞*
一个独立于主信息流的匿名社交玩法，模仿“漂流瓶”的经典交互。

*   **🌊 沉浸式海洋场景**：
    *   **视觉设计**：深海渐变背景 + CSS3 波浪视差动画 + 粒子悬浮效果，营造静谧的深海氛围。
    *   **交互动画**：精心调教的贝塞尔曲线动画 (`cubic-bezier`)，模拟漂流瓶被抛入海中逐渐远去、缩小的物理轨迹。

*   **🎲 随机邂逅机制**：
    *   **扔瓶子**：用户写下心事，匿名抛入“时间之海”。
    *   **捞瓶子**：基于 MySQL `ORDER BY RAND()` 实现的真·随机算法，让每一次打捞都是未知的惊喜。
    *   **匿名互动**：支持对捞到的瓶子进行点赞与评论，连接两个陌生灵魂的共鸣。
...
![image-20251224232434892](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242324299.png)

![image-20251224232455846](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242324337.png)

### 2. 首页  

![image-20251224232528684](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242325110.png)

### 3. 个人中心  

![image-20251224232559000](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242325095.png)

### 4. 树洞养料（签到）  

![image-20251224232633567](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242326042.png)

### 5. AI树洞  

![image-20251224232707902](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242327298.png)

### 6. 漂流树洞  

![image-20251224232735799](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242327348.png)

### 7. 时空胶囊  

![image-20251224232809090](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242328178.png)

### 8. 树洞来信  

![image-20251224232832334](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242328421.png)

### 9. 发布树洞  

![image-20251224232907851](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242329933.png)

### 10. 树洞密语

![image-20251224232942376](https://zlhimage.oss-cn-guangzhou.aliyuncs.com/202512242329759.png)
